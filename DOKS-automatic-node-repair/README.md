# Automatic Worker Node Repair in DOKS

When a node in a DOKS cluster is unhealthy or "NotReady,” replacing the node is manual and cumbersome. The cluster will operate at lower capacity without replacing the nodes because the unhealthy nodes will not run any Pods.
...
Hardware failures: If there is a hardware failure on the node (such as a disk or network interface failure), the node can become "NotReady."
...
We will deploy Digital Mobius on a running DOKS cluster for this tutorial. Digital Mobius is an open-source application written in Go specifically for DOKS cluster node recycling. The application monitors DOKS cluster nodes that are in an unhealthy state at specified regular intervals.
...
- [DigitalOcean access token](https://docs.digitalocean.com/reference/api/create-personal-access-token) for managing the DOKS cluster. Ensure that the access token has read-write scope.
...
<insert a multiple terminal window picture>
...
In this tutorial, we automatically recover cluster nodes if the kubelet service dies or becomes unresponsive. However, it’s also essential to avoid automatic node recycling by remembering to monitor node health and avoid overloading nodes effectively. In addition, following good practices for Pod resource limits, such as setting limits and using fair values, can help prevent high-load nodes.
