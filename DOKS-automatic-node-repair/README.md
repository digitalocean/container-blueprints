...
2. Fetch the cluster `ID` that you want to monitor for node failures:
    ...
    **Hint:**
    ...
    If you have only one cluster, then use `jq` (For example, on MacOS it can be installed with: `brew install jq`):
