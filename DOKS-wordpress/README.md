...
7. Basic knowledge on how to create and manage `Droplets`. You can learn more [here](https://docs.digitalocean.com/products/droplets).
...
The easiest way to install them is using DigitalOcean market place where you can install them using 1-Click App for [NGINX Ingress Controller](https://marketplace.digitalocean.com/apps/nginx-ingress-controller) and [Cert-Manager](https://marketplace.digitalocean.com/apps/cert-manager).

Create an cluster issuer. For convenience, this tutorial uses the Let's Encrypt production API. Create the following YAML file and replace <YOUR-EMAIL-HERE> with the contact email that you want the TLS certificate to show.
